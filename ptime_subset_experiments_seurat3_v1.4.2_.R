...
# Subsetting early and late hair cells.
...
script_name <- paste0("ptime_subset_experiments_seurat3_v1.4.2")
...
early_HC_subset <- which(cell_idents %in% c("early-HCs") &
  data_sets %in% c("homeo", "1.5hr", "2hr", "3hr", "5hr", "10hr"))

HC_subset <- which(cell_idents %in% c("mature-HCs") &
  data_sets %in% c("homeo", "1.5hr", "2hr", "3hr", "5hr", "10hr"))

seurat_obj <- seurat_obj[,c(early_HC_subset, HC_subset)]
...
p4 <- DimPlot(seurat_obj, reduction = "umap", pt.size = 0.70,
...
if(FALSE) {
  source(paste0("/n/projects/ddiaz/Analysis/Scripts/sb2191-regen/",
    "Seurat3-final/ptime_subset_experiments_seurat3_v1.4.2_.R"), echo = TRUE)
}
