...
# v1.4.1 adding homeo progenitors
{
library(Seurat)
library(WriteXLS)
library(ggplot2)
...
script_name <- paste0("ptime_subset_experiments_seurat3_v1.4.1")
...
central_subset <- which(cell_idents %in%
  c("central-cells") &
  data_sets %in% c(
    "homeo", "0min", "30min", "1hr", "1.5hr", "2hr", "3hr", "5hr"))
...
colors2 <- gg_color_hue(length(unique(seurat_obj@meta.data$cell.type.and.trt)) / 3)
...
png(figurePath(paste(
  "cell_type_and_trt_HCs_", "dim", max(dims),".png", sep = "_")),
...
if(FALSE) {
  source(paste0("/n/projects/ddiaz/Analysis/Scripts/sb2191-regen/",
    "Seurat3-final/ptime_subset_experiments_seurat3_v1.4.1_.R"), echo = TRUE)
}
