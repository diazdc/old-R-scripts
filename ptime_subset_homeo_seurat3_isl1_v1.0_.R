# Removes irg1l dominant cluster from analysis
...
script_name <- paste0("ptime_subset_homeo_seurat3_isl1_v1.0")
...
seurat_obj <- readRDS(paste0("/n/projects/ddiaz/Analysis/Data/",
  "sb2191-regen/SeurObj_ptime_subset_homeo_seurat3_v1.0_.RDS"))
seurat_obj@meta.data$old.tree.ident <- seurat_obj@meta.data$tree.ident
seurat_obj <- seurat_obj[,seurat_obj@meta.data$tree.ident != 4]
...
p3 <- DimPlot(seurat_obj, reduction = "umap", pt.size = 0.60,
...
# ==== Plot by old tree ident
p5 <- DimPlot(seurat_obj, reduction = "umap", pt.size = 0.60,
  label = TRUE, label.size = 4, group.by = "old.tree.ident")
p5 <- cleanUMAP(p5)

png(figurePath(paste(
  "clusters_old_tree_ident.png", sep = "_")),
  width = 12, height = 10, units = "in", res = 300)
print(p5)
dev.off()
...
if (FALSE) {
  testPCs(seurat_obj, from_to = 5:15, include_conditions = TRUE)
}
...
if(FALSE) {
  source(paste0("/n/projects/ddiaz/Analysis/Scripts/sb2191-regen/",
    "Seurat3-final/ptime_subset_homeo_seurat3_isl1_v1.0_.R"), echo = TRUE)
}
...
mcPlotMarkers(seurat_obj, marker_table, n_genes = 200,
  folder_prefix = "clusters_res0.4_")
...
if (FALSE) {
  saveRDS(obj_FitGoM_list,
    dataPath(paste0("FitGoM_obj_list_K6_through_K9_", script_name, ".RDS")))
...
stkd_vln_topic <- function (h) {
  ...
  png(figurePath(paste0(topic_dir, "topics", obj_FitGoM$fit$K,
    "_", n_p[h],"-", (n_p[h] + n_c), "_stacked_vln.png")), width = 8,
