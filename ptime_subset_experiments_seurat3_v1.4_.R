...
script_name <- paste0("ptime_subset_experiments_seurat3_v1.4")
...
prog_subset <- which(cell_idents %in% c("HC-prog") &
  data_sets %in% c("1hr", "1.5hr", "2hr", "3hr", "5hr", "10hr"))
...
if(FALSE) {
  source(paste0("/n/projects/ddiaz/Analysis/Scripts/sb2191-regen/",
    "Seurat3-final/ptime_subset_experiments_seurat3_v1.4_.R"), echo = TRUE)
}
...
# head(print.data.frame(group_by(seurat_obj@meta.data, tree.ident)))
...
seurat_obj@meta.data %>%
  group_by(tree.ident) %>%
  summarise(topic1 = mean(topic1), topic2 = mean(topic2), topic3 = mean(topic3),
    topic4 = mean(topic4), topic5 = mean(topic5), topic6 = mean(topic6),
    topic7 = mean(topic7), topic8 = mean(topic8), topic9 = mean(topic9),
    topic10 = mean(topic10))

seurat_obj@meta.data %>%
  group_by(cell.type.and.trt) %>%
  summarise(topic1 = mean(topic1), topic2 = mean(topic2), topic3 = mean(topic3),
    topic4 = mean(topic4), topic5 = mean(topic5), topic6 = mean(topic6),
    topic7 = mean(topic7), topic8 = mean(topic8), topic9 = mean(topic9),
    topic10 = mean(topic10), topic11 = mean(topic11), topic12 = mean(topic12),
    topic13 = mean(topic13), topic14 = mean(topic14), topic15 = mean(topic15))


# topic_mean_list <- lapply(1:obj_FitGoM$fit$K, function(i) {
#   mean(seurat_obj@meta.data[,paste0("topic", i)])
# })

# names(topic_mean_list) <- lapply(1:obj_FitGoM$fit$K, function(i) {
#   paste0("topic", i)
# })
