...
script_name <- paste0("ptime_subset_experiments_seurat3_v1.5")
...
HC_subset <- which(cell_idents %in% c("mature-HCs") &
  data_sets %in% c("2hr", "3hr", "5hr", "10hr"))
...
ind <- grep("early|mature", unique(seurat_obj@meta.data$cell.type.and.trt))
...
if(FALSE) {
  source(paste0("/n/projects/ddiaz/Analysis/Scripts/sb2191-regen/",
    "Seurat3-final/ptime_subset_experiments_seurat3_v1.5_.R"), echo = TRUE)
}
stop("End Analysis")

to_replace <- grep("early-HCs", levels(seurat_obj@meta.data$cell.type.and.trt), value = TRUE)
replacement <- gsub("early","young", to_replace)

seurat_obj@meta.data$cell.type.and.trt <- plyr::revalue(
  seurat_obj@meta.data$cell.type.and.trt, c(
    c("1.5hr.early-HCs","2hr.early-HCs", "3hr.early-HCs", "5hr.early-HCs", "10hr.early-HCs") =
    c("1.5hr.young-HCs", "2hr.young-HCs", "3hr.young-HCs", "5hr.young-HCs", "10hr.young-HCs")))
