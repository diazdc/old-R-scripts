...
script_name <- paste0("ptime_subset_experiments_seurat3_v1.3")
...
if(FALSE) {
  source(paste0("/n/projects/ddiaz/Analysis/Scripts/sb2191-regen/",
    "Seurat3-final/ptime_subset_experiments_seurat3_v1.3_.R"), echo = TRUE)
}
...
print(cowplot::plot_grid(plotlist = topic_umap[1:15], ncol = 3))
dev.off()
}
...
WriteXLS::WriteXLS(topic_gene_df,
  figurePath(paste0("topics_", obj_FitGoM$fit$K,"_gene_table.xlsx")),
  row.names = FALSE, col.names = TRUE, AdjWidth = TRUE)
}
...
omega <- data.frame(obj_FitGoM$fit$omega)
annotation <- data.frame(sample_id = rownames(omega),
  tissue_label = paste0("cluster", seurat_obj@active.ident))
...
stkd_vln_topic <- function (h) {
  plot_list <- lapply(n_p[h]:(n_p[h] + 4), function(i) {
  ...
  png(figurePath(paste0("topics", obj_FitGoM$fit$K, "_", n_p[h],"-",
    (n_p[h] + 4), "_stacked_vln.png")), width = 15,
